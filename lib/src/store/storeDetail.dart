...
import 'package:dbapp/src/store/storePurchase.dart';
import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:flutter_money_formatter/flutter_money_formatter.dart';
import 'package:provider/provider.dart';
import 'package:dbapp/src/store/storeMakeBottom.dart';
        ...
        body: Center(
          ...
          child: Column(
            children: <Widget>[
              Container(
                alignment: Alignment.center,
                child: Image.network(widget.document['img'],
                          ...
                          InkWell(
                            ...
                            onTap: () {
                              if (widget.array
                                ...
                                if (widget.count == 0) {
                                  _showDialog2(
                                      "0개의 상품이 선택되었습니다. 1개이상 상품을 선택해주세요");
                                } else {
                                  // 처음추가
                                  widget.array.putIfAbsent(
                                      widget.document['name'],
              ...
              Expanded(
                child: Container(
                  ...
                  child: InkWell(
                    onTap: () {
                      // 결제하기
                      if (widget.array.containsKey(widget.document['name'])) {
                        //맵에 키가 존재
                        if (widget.count > 0) {
                          // 개수 1개 이상으로 변경
                          // 이미 추가한적이 있다 -> 최신화
                          ...
                          widget.money.putIfAbsent(widget.document['name'],
                              () => widget.document['price']);
                        } else {
                          // 개수 0개로 변경 -> 삭제
                          // 이미 추가한적이 있다 -> 최신화다
...
//                            _showDialog();
                      ...
                      if (widget.array.keys.toList().length != 0) {
                        Navigator.of(context).push(MaterialPageRoute(
                            builder: (context) => FoodPurchase(
                                select: widget.array, money: widget.money)));
                      } else {
